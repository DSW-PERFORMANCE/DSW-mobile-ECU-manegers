...
class StorageManager {
     ...
     * Usa ConfigMacros se disponível para determinar armazenamento por ambiente
     */
    async save(key, data) {
        try {
            const payload = {
            ...
            // Usar ConfigMacros se disponível
            if (window.ConfigMacros) {
                return await window.ConfigMacros.choose({
                    browser: () => this._saveBrowser(key, payload),
                    webview: async () => await this._saveWebView(key, payload),
                    windows: async () => await this._saveWindows(key, payload)
                });
            }

            // Fallback para switch tradicional se ConfigMacros não estiver carregado
     ...
     * Usa ConfigMacros se disponível para determinar carregamento por ambiente
     */
    async load(key) {
        try {
            let payload;
            ...
            // Usar ConfigMacros se disponível
            if (window.ConfigMacros) {
                payload = await window.ConfigMacros.choose({
                    browser: () => this._loadBrowser(key),
                    webview: async () => await this._loadWebView(key),
                    windows: async () => await this._loadWindows(key)
                });
            } else {
                // Fallback para switch tradicional se ConfigMacros não estiver carregado
                switch (this.environment) {
    ...
    /**
     * Remove dados
     * Usa ConfigMacros se disponível para determinar remoção por ambiente
     */
    async remove(key) {
        try {
            // Usar ConfigMacros se disponível
            if (window.ConfigMacros) {
                return await window.ConfigMacros.choose({
                    browser: () => this._removeBrowser(key),
                    webview: async () => await this._removeWebView(key),
                    windows: async () => await this._removeWindows(key)
                });
            }
